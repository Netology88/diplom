...
resource "local_file" "ansible-inventory" {
  content  = <<-EOT
    ...
    [all:vars]
    ansible_ssh_common_args='-o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o ProxyCommand="ssh -p 22 -W %h:%p -q ubunta@${yandex_compute_instance.bastion-host.network_interface.0.nat_ip_address}"'
    EOT
  filename = "/home/matvey/Diplom2/ansible/inventory.ini"
}
